...
public class GridManager : MonoBehaviour
{
    ...
    public void InitializeGrid(Vector2Int size)
    {
        ...
        for (int x = 0; x < gridSize.x; x++)
        {
            for (int y = 0; y < gridSize.y; y++)
            {
                Vector3 cellPosition = transform.position + transform.forward * (offset.x + x * cellSize) -
                                       transform.right * (offset.y + y * cellSize);
                Quaternion rotationOffset =
                    Quaternion.AngleAxis(Random.Range(-rotationJitter, rotationJitter), Vector3.up);
                Vector3 positionOffset = Vector3.right * Random.Range(-positionJitter, positionJitter) +
                                         Vector3.forward * Random.Range(-positionJitter, positionJitter);
                GridCell cell = Instantiate(cellPrefab, cellPosition + positionOffset,
                    Quaternion.identity * rotationOffset, cellHolder);
